...
A powerful command-line tool for FERS retirement planning with both interactive and scriptable interfaces.
...
./fers-calc monte-carlo config.yaml --interactive
```
...
- `-i, --interactive` - Show live progress with Bubble Tea UI
...
Interactive wizard to create a new configuration file:

```bash
# Launch wizard (saves to config.yaml by default)
./fers-calc wizard

# Save to custom file
./fers-calc wizard --output my-retirement.yaml
```

The wizard guides you through:
- Personal details (birth date, hire date, salary)
- TSP balances and allocations
...
./fers-calc monte-carlo myplan.yaml --runs 5000 --interactive
...
The interactive mode (`./fers-calc` with no args) provides:

- ✨ Beautiful terminal UI with Bubble Tea
- 📊 Main menu for all operations
- 🧙 Interactive configuration wizard
- 📈 Live progress bars for Monte Carlo simulations
- ⌨️ Keyboard navigation (arrow keys, Enter, q to quit)

## Next Steps

1. Run `./fers-calc wizard` to create your first configuration
2. Review and edit the generated YAML file
3. Run calculations: `./fers-calc calculate config.yaml`
4. Generate reports in HTML format for easy sharing
